# Gemini Deep Research Prompt (Concise Version)

Copy this entire prompt into Gemini Deep Research:

---

## Research Objective
Analyze and optimize a Gemini API prompt for extracting structured workout data from whiteboard photos. Improve accuracy, consistency, and reliability of extracting CrossFit workout information.
...
1. **Prompt Structure**: Is the prompt too long? Should we use few-shot examples? Chain-of-thought reasoning?
2. **Model Selection**: Which Gemini model (2.5 Pro, 2.5 Flash, 2.5 Flash-Lite, 2.0 Flash) performs best?
3. **Prompt Engineering**: Would structured output mode help? Temperature=0? Function calling?
4. **Alternative Approaches**: Should we use OCR first then parse? Two-stage extraction?
5. **Schema Optimization**: Is the JSON schema optimal? Different output format?
6. **Error Handling**: How to make prompt more robust to edge cases?

## Expected Deliverables

1. **Prompt Analysis**: Identify weaknesses, suggest improvements with rationale
2. **Model Comparison**: Test on different Gemini models, compare accuracy/cost/speed
3. **Alternative Approaches**: Research alternative architectures, evaluate trade-offs
4. **Optimized Prompt**: Provide improved prompt versions with test results
5. **Best Practices Report**: Summarize findings with actionable recommendations

## Testing Criteria

Evaluate improvements on:
