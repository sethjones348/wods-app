...
async function generatePdf(testDir: string, outputPath: string): Promise<void> {
    ...
    // Title page
    doc.fontSize(24).font('Helvetica-Bold').text('Test Results: New Workout Extraction Prompt', { align: 'center' });
    doc.moveDown(1);
...
async function main() {
    ...
    if (!testDir) {
        console.error('Usage: tsx scripts/generate-test-pdf.ts <test-directory>');
        console.error('Example: tsx scripts/generate-test-pdf.ts test-new-prompt-2025-12-25T00-29-08');
        process.exit(1);
    }

    const fullTestDir = path.isAbsolute(testDir) ? testDir : path.join(process.cwd(), testDir);
